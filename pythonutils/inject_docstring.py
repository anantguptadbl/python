...
expr = ast.parse(open("sample_code_wthout_class.py").read())
expr_ast = ast.parse(expr)
print(ast.dump(expr_ast))
unmodified = ast.parse(expr)
exec(compile(unmodified, '<string>', 'exec'))
...
with open("sample_code_modified.py", "w") as f:
    f.write(astunparse.unparse(modified))
