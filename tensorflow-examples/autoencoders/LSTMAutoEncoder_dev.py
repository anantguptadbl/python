...
# Input Placeholders
x=tf.placeholder(shape=[None,step_num,elem_num],dtype=tf.float32)
y=tf.placeholder(shape=[None,step_num,elem_num],dtype=tf.float32)
...
#hiddenState=tf.reshape(hiddenState,[5,hidden_num])

def getOutput(hiddenState):
    output=tf.nn.softmax(tf.matmul(hiddenState,outputHiddenStateWeight) + outputHiddenStateBias)
    return output


output=tf.map_fn(getOutput,states[:, 0, :, :][0])
...
input_x=np.random.randint(2,size=(100,5,1)).astype(np.float32)
input_y=np.random.randint(2,size=(100,5,1)).astype(np.float32)

with tf.Session() as sess:
    for epoch in range(100):
        print(sess.run(loss, feed_dict={x: input_x, y: input_x}))
